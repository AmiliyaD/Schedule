...
class TeacherMap extends BaseMap {
    ...
    {
        $teach = $this->db->query("SELECT * FROM teacher INNER JOIN user ON teacher.user_id = user.user_id INNER JOIN gender ON user.gender_id = gender.id INNER JOIN otdel ON otdel.id = teacher.otdel_id INNER JOIN role ON role.id = user.role_id LIMIT 10")->fetchAll();
        $teachers = $this->db->query("SELECT * FROM user INNER JOIN role ON user.role_id = role.id INNER JOIN gender ON user.gender_id = gender.id INNER JOIN teacher ON teacher.user_id = user.user_id  INNER JOIN otdel ON teacher.otdel_id = otdel.id WHERE role.name = 'Преподаватель' ")->fetchAll();
        return $teach;
    }
    // функция 1.3. количество
    public function count()
    {
        $res = $this->db->query("SELECT COUNT(*) AS cnt FROM teacher")->fetch(PDO::FETCH_ASSOC)->cnt;
        return $res;
    }
